...
st.image(image, caption='Buchwald-Hartwig-type C-N cross-coupling reactions')

def plot_graph(data):
	plt.rcParams['font.sans-serif']=['SimHei']
	plt.rcParams['axes.unicode_minus']=False
	a = np.linspace(-0.1, 100,1000)
	plt.scatter(data['Actual'].values, data['Predicted'].values,label='数据点', alpha=0.5, cmap='viridis')
	plt.plot(a,a, label='对角线', ls='--', c='k')
	plt.text(80, 20, '$R^2$=0.94',fontsize=15, horizontalalignment='center', verticalalignment='center')
	plt.title('验证集', fontsize=15)
...
if prediction:
	...
	st.markdown("===================================================================================================================================")
	st.markdown('<div style="border: 2px solid #4908d4; border-radius:20px; padding:3%; text-align:center"> <h5> 开发人员肝了一整天，目前有很多BUG，大家凑合着用后续看心情更新</h5> </div>',unsafe_allow_html=True)